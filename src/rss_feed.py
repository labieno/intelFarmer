...
def update_database_json():
    try:
        ...
        pass # Most likely means the folder already exists

    for source in rss_feed_list:
        filename = f"logs/{source[1]}.json"
        try:
            # If logs/{source[1]}.json does NOT exists, dump all entries
            if (not os.path.exists(filename)) or (os.path.getsize(filename) <= 2):
            ...
            else: # If logs/{source[1]}.json exists, search for reports in current month (could be changed)
                print("========================================", source[1], "========================================")
                ...
                with open(filename, 'r') as infile:
                    ...
                    try:
                        ...
                        if new_articles: # Dump new articles to json files
...
def check_source(s):
    ...
    if url != '':
        ...
        print("========================================", source[1], "========================================")
        if feed.entries == []:
            print(f"        [+] No articles published by {source[1]}")
        ...
        print(f"[-] {s} is not in feed list, check if the source name provided is correct.")
